...
import { AlienWorldsAccount, AlienWorldsAccountsTableRow, DacDirectory, DacGlobals, DacsTableRow, Stat } from '@alien-worlds/eosdac-api-common';
...
const getAllDacsUseCase = {
  execute: jest.fn(() => Result.withContent([DacDirectory.fromTableRow(<DacsTableRow>{
    accounts: [{ key: 0, value: 'dao.worlds' }, { key: 2, value: 'dao.worlds' }],
    symbol: { sym: 'EYE' },
    refs: [],
  })]))
};
const getDacTreasuryUseCase = {
  execute: jest.fn(() => Result.withContent([AlienWorldsAccount.fromTableRow(<AlienWorldsAccountsTableRow>{
    balance: 'string',
  })]))
};
const getDacInfoUseCase = {
  execute: jest.fn(() => Result.withContent([DacGlobals.fromTableRow({
    data: [
      {
...
const getDacTokensUseCase = {
  execute: jest.fn(() => Result.withContent([Stat.fromTableRow({
...
describe('GetDacs Controller Unit tests', () => {
  beforeAll(() => {
    ...
    /*bindings*/
    container.bind<GetAllDacsUseCase>(GetAllDacsUseCase.Token).toConstantValue(getAllDacsUseCase as any);
    container.bind<GetDacTreasuryUseCase>(GetDacTreasuryUseCase.Token).toConstantValue(getDacTreasuryUseCase as any);
    container.bind<GetDacInfoUseCase>(GetDacInfoUseCase.Token).toConstantValue(getDacInfoUseCase as any);
    container.bind<GetDacTokensUseCase>(GetDacTokensUseCase.Token).toConstantValue(getDacTokensUseCase as any);
    container.bind<GetDacsController>(GetDacsController.Token).to(GetDacsController);
  });
  ...
  it('Should return failure when GetAllDacsUseCase fails', async () => {
    getAllDacsUseCase.execute.mockImplementationOnce(jest.fn(() => Result.withFailure(Failure.fromError(null))))
  ...
  it('Should return failure when GetDacTreasuryUseCase fails', async () => {
    getDacTreasuryUseCase.execute.mockImplementationOnce(jest.fn(() => Result.withFailure(Failure.fromError(null))))
  ...
  it('Should return failure when GetDacInfoUseCase fails', async () => {
    getDacInfoUseCase.execute.mockImplementationOnce(jest.fn(() => Result.withFailure(Failure.fromError(null))))
  ...
  it('Should return failure when GetDacTokensUseCase fails', async () => {
    getDacTokensUseCase.execute.mockImplementationOnce(jest.fn(() => Result.withFailure(Failure.fromError(null))))
