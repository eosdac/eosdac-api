...
import { AlienWorldsAccount } from '@alien-worlds/eosdac-api-common';
...
export class GetDacsController {
  ...
  constructor(
    /*injections*/
    @inject(GetAllDacsUseCase.Token) private getAllDacsUseCase: GetAllDacsUseCase,
    @inject(GetDacTreasuryUseCase.Token) private getDacTreasuryUseCase: GetDacTreasuryUseCase,
    @inject(GetDacInfoUseCase.Token) private getDacInfoUseCase: GetDacInfoUseCase,
    @inject(GetDacTokensUseCase.Token) private getDacTokensUseCase: GetDacTokensUseCase
  ) { }
  ...
  public async dacs(input: GetDacsInput): Promise<Result<GetDacOutput[]>> {
    ...
    const { content: dacs, failure: getAllDacsFailure } = await this.getAllDacsUseCase.execute(input);
    ...
    for (const dac of dacs) {
      let dacTreasury: AlienWorldsAccount;
      ...
      output.push(GetDacOutput.create(dac, dacTreasury, dacInfo[0], dacTokens[0]))
    }
