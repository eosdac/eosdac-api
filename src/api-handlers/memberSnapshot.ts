...
async function memberSnapshot(fastify, request) {
	...
	return new Promise(async (resolve, reject) => {
		...
		const symbol =
			...
			(dac_config.symbol.sym || dac_config.symbol.symbol).split(',')[1];
		const precision = parseInt(
			(dac_config.symbol.sym || dac_config.symbol.symbol).split(',')[0]
		);
		const block_num = request.query.block_num || null;
		const sort_col = request.query.sort || 'account';

		fastify.log.info(
			`Generating snapshot for ${symbol}@${contract} on block ${block_num} with dac_id ${dac_id}`,
			{ dac_id }
		);
		...
		while (has_more) {
			...
			members_res.results.forEach(member => {
				...
				members.set(member.data.sender, {
					...
					zero_balance: true,
					balance: [zero_bal, symbol],
					voter: false,
					block_num: member.block_num,
				});
			});
			fastify.log.info(`Members results length ${members_res.count}`, {
				dac_id,
			});
		...
		const balances_match: any = {
			...
			limit,
			skip,
			scope: { $in: Array.from(members.keys()) },
		};
		if (block_num) {
			...
			if (members.has(custodian.data.candidate_name)) {
				...
				if (!member_data.balance) {
					...
					member_data.balance[0] = (
						parseFloat(member_data.balance[0]) + parseFloat(stake[0])
					).toFixed(precision);
				}
