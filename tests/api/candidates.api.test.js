...
const fixtures = require('../fixtures/candidates.fixture');
...
describe('Candidates API Test', () => {
  ...
  describe('limit parameter', () => {
    ...
    it('should return all items when limit is greater than or equal to count of available items', async () => {
      const response = await getApiResponse(Api.method, `${Api.url}?limit=${fixtures.candidatesResponse.count}`)

      const jsonResponse = JSON.parse(response.body)
      expect(jsonResponse.results.length).toEqual(fixtures.candidatesResponse.count);
    });
    ...
    it('should use default limit when provided limit is zero', async () => {
      ...
      const jsonResponse = JSON.parse(response.body)
      expect(jsonResponse.results.length).toBeLessThanOrEqual(LIMIT_DEFAULT_VALUE);
    });
  ...
  describe('skip parameter', () => {
    it('should skip items equal to skip parameter', async () => {
      ...
      const jsonResponse = JSON.parse(response.body)
      expect(jsonResponse.results[0]).toEqual(fixtures.candidatesResponse.results[skipParam]);
      expect(jsonResponse.results.length).toEqual(fixtures.candidatesResponse.count - skipParam);
    });
    ...
    it('should return empty list when skip value is equal to available items', async () => {
      const response = await getApiResponse(Api.method, `${Api.url}?skip=${fixtures.candidatesResponse.count}`)
    ...
    it('should use default value when skip parameter is not provided', async () => {
      ...
      expect(jsonResponse.results[SKIP_DEFAULT_VALUE]).toEqual(fixtures.candidatesResponse.results[SKIP_DEFAULT_VALUE]);
      expect(jsonResponse.results.length).toEqual(fixtures.candidatesResponse.count - SKIP_DEFAULT_VALUE);
    });
