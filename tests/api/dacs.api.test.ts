...
describe('Dacs API Test', () => {
	...
	it('should return list of dacs', async () => {
		const response = await getApiResponse(Api.method, `${Api.url}?limit=${Data.Limit}`);
	...
	describe('dacId param', () => {
		it('should return only specified dac', async () => {
			const response = await getApiResponse(Api.method, `${Api.url}?dacId=${Data.DacId}`);
			const jsonResponse = JSON.parse(response.body);
		...
		it('should return status code 404 for invalid dacId', async () => {
			const response = await getApiResponse(Api.method, `${Api.url}?dacId=dummy`);
		...
		it('should return error for invalid dacId', async () => {
			...
			expect(jsonResponse.error).toBe("Data with dummy in scope: 'index.worlds' table: 'dacs' scope not found");
		});
	});
	...
	describe('should return each individual dac', () => {
		test.each(Data.AllDacs)("should return dac '%s'", async (dacId) => {
			const response = await getApiResponse(Api.method, `${Api.url}?dacId=${dacId}`);
			const jsonResponse = JSON.parse(response.body);
