...
describe('Balance timeline API Test', () => {
  it('should return status 200', async () => {
    const response = await getApiResponse(Api.method, `${Api.url}?account=${Data.Account}&contract=${Data.Contract}&symbol=${Data.Symbol}`)
  ...
  it('should return success reponse', async () => {
    ...
    expect(JSON.parse(response.body)).toEqual(fixtures.balanceTimelineOneItemResponse)
  });
  ...
  describe('start and end block params', () => {
    ...
    describe('start block param', () => {
      it('should filter results greater than or equal to start block number', async () => {
        ...
        const response = await getApiResponse(Api.method, `${Api.url}?account=${Data.Account}&contract=${Data.Contract}&symbol=${Data.Symbol}&start_block=${startBlockParam}`)
        const jsonResp = JSON.parse(response.body)

        expect(jsonResp.count).toEqual(fixtures.balanceTimelineStartParamResponse.count)
        expect(jsonResp.results.slice(0, -1)).toEqual(fixtures.balanceTimelineStartParamResponse.results.slice(0, -1))
      });
    });
    ...
    describe('end block param', () => {
      it('should filter results less than or equal to start block number', async () => {
        ...
        const response = await getApiResponse(Api.method, `${Api.url}?account=${Data.Account}&contract=${Data.Contract}&symbol=${Data.Symbol}&end_block=${endBlockParam}`)
        const jsonResp = JSON.parse(response.body)

        expect(jsonResp.count).toEqual(fixtures.balanceTimelineEndParamResponse.count)
        expect(jsonResp.results.slice(0, -1)).toEqual(fixtures.balanceTimelineEndParamResponse.results.slice(0, -1))
      });
    });
  });
});
