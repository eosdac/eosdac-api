...
const fixtures = require('../fixtures/votes_timeline.fixture');
...
describe('Votes timeline API Test', () => {
  it('should return status 200', async () => {
    const response = await getApiResponse(Api.method, `${Api.url}?account=${Data.Accounts[0]}`)
  ...
  it('should return empty list when no matching results are available', async () => {
    ...
    expect(JSON.parse(response.body)).toEqual(fixtures.votesTimelineEmptyResponse);
  });
  ...
  it('should accept multiple accounts', async () => {
    const response = await getApiResponse(Api.method, `${Api.url}?account=${Data.Accounts[0]},${Data.Accounts[1]}`)
