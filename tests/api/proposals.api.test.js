...
const fixtures = require('../fixtures/proposals.fixture');
...
describe('Proposals API Test', () => {
  ...
  it('should return proposals list', async () => {
    ...
    expect(new Set(jsonResp.results)).toEqual(new Set(fixtures.allProposalsList.results));
  ...
  describe('status query parameter', () => {
    it('should return proposals with matching status', async () => {
      ...
      const jsonResp = JSON.parse(response.body)
      expect(new Set(jsonResp.results)).toEqual(new Set(fixtures.status0ProposalsList.results));
    });

    it('should accept multiple status values and return matching proposals', async () => {
      const response = await getApiResponse(Api.method, `${Api.url}?status=0,1`)

      const jsonResp = JSON.parse(response.body)

      expect(new Set(jsonResp.results)).toEqual(new Set(fixtures.status0_1_ProposalsList.results));
    ...
    it('should return empty response for invalid status', async () => {
      ...
      expect(JSON.parse(response.body)).toEqual(fixtures.proposalsEmptyResponse);
    });
  ...
  describe('limit query parameter', () => {
    ...
    it('should return all items when limit is greater than or equal to count of available items', async () => {
      const response = await getApiResponse(Api.method, `${Api.url}?limit=${fixtures.allProposalsList.count}`)
  ...
  describe('skip query parameter', () => {
    ...
    it('should skip items equal to skip parameter', async () => {
      ...
      const jsonResp = JSON.parse(response.body)
      expect(jsonResp.results.length).toEqual(fixtures.allProposalsList.count - skipParam);
    });
    ...
    it('should not skip items when skip parameter is zero', async () => {
      ...
      expect(jsonResp.results[0]).toEqual(fixtures.allProposalsList.results[skipParam]);
      expect(jsonResp.results.length).toEqual(fixtures.allProposalsList.count);
    });

    it('should return empty list when skip value is equal to available items', async () => {
      const response = await getApiResponse(Api.method, `${Api.url}?skip=${fixtures.allProposalsList.count}`)
    ...
    it('should return empty result but valid count when skip parameter is negative', async () => {
      ...
      expect(JSON.parse(response.body)).toEqual(fixtures.proposalsNegativeSkipParamResponse);
    });
    ...
    it('should use default value when skip parameter is not provided', async () => {
      ...
      expect(jsonResp.results[0]).toEqual(fixtures.allProposalsList.results[SKIP_DEFAULT_VALUE]);
      expect(jsonResp.results.length).toEqual(fixtures.allProposalsList.count - SKIP_DEFAULT_VALUE);
    });
